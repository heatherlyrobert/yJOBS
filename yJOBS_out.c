...
yjobs_out_full          (char a_runas, char a_mode, char a_oneline [LEN_HUND], char a_file [LEN_PATH], void *f_callback)
{
   ...
   /*---(prepare)------------------------*/
   rc = yjobs__out_prepare (a_runas, a_mode, a_oneline, a_file, f_callback, NULL, x_world, x_db, x_full);
   --rce;  if (rc < 0) {
      DEBUG_YJOBS   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(read database)------------------*/
   /*> rc = yjobs__out_readdb   (a_mode, x_db, f_callback);                           <* 
    *> --rce;  if (rc < 0) {                                                          <* 
    *>    DEBUG_YJOBS   yLOG_exitr   (__FUNCTION__, rce);                             <* 
    *>    return rce;                                                                 <* 
    *> }                                                                              <*/

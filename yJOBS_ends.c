...
char
yjobs__ends_cwd         (char a_mode, char a_name [LEN_TERSE], char a_cdir [LEN_PATH], char r_cwd [LEN_PATH], char r_full [LEN_PATH])
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        x_cwd       [LEN_PATH]  = "";
   ...
   if (r_full  != NULL) {
      ...
      else if (strchr (g_central, a_mode) != NULL)   sprintf (r_full , "%s/%s", a_cdir, a_name);
   }
...
char
yjobs_ends_header       (cchar a_runas, cchar a_mode, cchar a_oneline [LEN_HUND], cchar a_name [LEN_TERSE], char r_cdir [LEN_DESC], char r_world [LEN_LABEL], char *r_update, char r_db [LEN_LABEL], char r_cwd [LEN_PATH], char r_full [LEN_PATH])
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
...
char
yjobs_ends_footer       (cchar a_mode)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
...
char
yjobs_ends_failure      (cchar a_mode, cchar *a_text)
{
   /*---(header)-------------------------*/
   DEBUG_YJOBS  yLOG_enter   (__FUNCTION__);
   DEBUG_YJOBS  yLOG_char    ("m_mode"    , a_mode);

...
yjobs__in_dir           (char a_runas, char a_mode, char a_file [LEN_PATH], char r_fuser [LEN_USER], char r_fdir [LEN_PATH], char r_full [LEN_PATH])
{
   ...
   yURG_msg ('-', "directory name å%sæ is not null or empty", a_file);
   DEBUG_YJOBS   yLOG_info    ("a_file"    , a_file);
   --rce;  if (strcmp (a_file, "dir") == 0) {
      yURG_msg ('-', "requested ådiræ for current directory (preferred)");
   } else  if (strcmp (a_file, "/")   == 0) {
      yURG_msg ('-', "requested å/æ for current directory (preferred)");
   } else  if (a_file [0] == '/') {
      yURG_msg ('-', "requested specific directory (not allowed, ignored)");
   } else {
      yURG_err ('f', "did not request directory with ådiræ, å/æ, or absolute path");
      DEBUG_YJOBS   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(get security data)--------------*/
   ...
   yURG_msg ('-', "using å%sæ and å%sæ as allowed directory prefixes", myJOBS.m_root, myJOBS.m_home);
   /*---(check directory)----------------*/
   ...
   yURG_msg ('-', "current working directory is å%sæ", x_fdir);
   /*---(filter locations)---------------*/
   --rce;  if (strncmp (x_fdir, myJOBS.m_root, strlen (myJOBS.m_root)) == 0) {
      ...
      yURG_msg ('-', "ownership is å%sæ (%d)", x_fuser, x_uid);
...
char
yjobs__in_verify        (char a_runas, char a_mode, char a_dir [LEN_PATH], char a_file [LEN_PATH], char r_full [LEN_PATH], char r_fuser [LEN_USER])
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   int         rc          =    0;
   ...
   DEBUG_YJOBS   yLOG_complex ("a_args"    , "%c %c", a_runas, a_mode);
   /*---(score)--------------------------*/
   g_acts_score  [G_SCORE_LAUDIT ] = G_SCORE_FAIL;
   /*---(default)------------------------*/
   if (r_full   != NULL)  strcpy (r_full  , "");
   if (r_fuser  != NULL)  strcpy (r_fuser , "");
   /*---(defense)------------------------*/
   ...
   DEBUG_YJOBS   yLOG_point   ("a_file"    , a_file);
   --rce;  if (a_file == NULL) {
      yjobs_ends_failure (a_mode, "local file name is null");
      DEBUG_YJOBS   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   DEBUG_YJOBS   yLOG_info    ("a_file"    , a_file);
   ...
   --rce;  if (strcmp (a_file, "") == 0) {
      ...
      /*---(header)----------------------*/
      yURG_msg ('>', "local directory verification...");
      /*---(verify)----------------------*/
      ...
      /*---(score)--------------------------*/
      g_acts_score  [G_SCORE_LAUDIT ] = 'd';
      DEBUG_YJOBS   yLOG_value   ("score"     , rc);
      /*---(finish)----------------------*/
      yURG_msg ('-', "success, local directory acceptable");
      /*---(done)------------------------*/
   }
   /*---(file)---------------------------*/
   else {
      ...
      /*---(verify)----------------------*/
      rc = yENV_audit_local (a_dir, a_file, x_prefix, x_suffix, r_full, NULL, r_fuser);
...
yjobs__in_pull          (char a_mode, void *f_callback, char a_full [LEN_PATH])
{
   ...
   */
   ...
   DEBUG_YJOBS   yLOG_enter   (__FUNCTION__);
   /*---(score)--------------------------*/
   g_acts_score  [G_SCORE_LPULL  ] = G_SCORE_FAIL;
   DEBUG_YJOBS   yLOG_value   ("pre-score" , rc);
   /*---(check call-back)----------------*/
   DEBUG_YJOBS   yLOG_point   ("callback"  , f_callback);
   --rce;  if (f_callback == NULL) {
...
yjobs__in_report        (char a_mode, void *f_callback, char a_full [LEN_PATH])
{
   ...
   DEBUG_YJOBS   yLOG_enter   (__FUNCTION__);
   /*---(score)--------------------------*/
   g_acts_score  [G_SCORE_LRPT   ] = G_SCORE_FAIL;
   /*---(default)------------------------*/
...
yjobs__in_intake        (char a_runas, char a_mode, char a_file [LEN_PATH], char a_db [LEN_LABEL], char a_fuser [LEN_USER])
{
   ...
   g_acts_score  [G_SCORE_CMOVE  ] = G_SCORE_FAIL;
   /*---(get central info)-------------------*/
   ...
   /*---(summary)----------------------------*/
   yURG_msg ('-', "success, configuration file moved to central");
   g_acts_score  [G_SCORE_CMOVE  ] = 'm';
   /*---(complete)-----------------------*/
   DEBUG_YJOBS   yLOG_exit    (__FUNCTION__);
   return 0;
}
...
char
yjobs__in_register      (char a_runas, char a_mode, char a_file [LEN_PATH], char a_hdir [LEN_DESC], char a_world [LEN_LABEL], void *f_testcall)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   int         rc          =    0;
...
yjobs_in_full           (char a_runas, char a_mode, char a_oneline [LEN_HUND], char a_file [LEN_PATH], void *f_callback)
{
   ...
   DEBUG_YJOBS   yLOG_enter   (__FUNCTION__);
   /*---(prepare)------------------------*/
   rc = yjobs_share_prepare ("yjobs__in_prepare", 'i', a_runas, a_mode, a_oneline, a_file, f_callback, NULL, x_hdir, x_world, x_db, x_cwd, x_full);
   ...
   /*---(read database)------------------*/
   rc = yjobs_share_readdb   ("yjobs__in_readdb", 'i', a_mode, x_hdir, x_db, f_callback);
   ...
   DEBUG_YJOBS   yLOG_value   ("rc_final"  , rc_final);
   /*---(verify local)-------------------*/
   rc = yjobs__in_verify   (a_runas, a_mode, x_cwd, a_file, x_full, x_fuser);
   ...
   DEBUG_YJOBS   yLOG_value   ("rc_final"  , rc_final);
   /*---(pull local)---------------------*/
   rc = yjobs_share_writedb  ("yjobs__in_writedb", 'i', a_mode, x_hdir, x_db, f_callback);
   ...
   DEBUG_YJOBS   yLOG_value   ("rc_final"  , rc_final);
   /*---(register)-----------------------*/
   rc = yjobs__in_register (a_runas, a_mode, x_full, x_hdir, x_world, NULL);
   DEBUG_YJOBS   yLOG_value   ("register"  , rc);
   --rce;  if (rc < 0) {
      DEBUG_YJOBS   yLOG_exitr   (__FUNCTION__, rce);
      return rce;

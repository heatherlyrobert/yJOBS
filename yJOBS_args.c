 ...
 *
 *      
 *       É∑∑∑∑∑∑∑∑∑∑∑∑∑∑ yJOBS standard request set (28) ∑∑∑∑∑∑∑∑∑∑∑∑∑∑∑∑Ç
 *            
 *                                   ÉÄÄÄÄÄÄÄÄÄÇ
 *                                   å version å
 *                                   å about   å
 ...
 *           å  Å verify   Å              å                         å
 *           å  Å local    Å              å           É∑∑∑∑∑∑∑∑∑∑Ç  å
 *           å  Å register áÄÄ>>>ÄÄÄÄÄÄÄÇ å ÉÄÄÄÄ>>>ÄÄÜ backup   Å  å
 *           å  Å update   Å         ÉÄÄàÄàÄàÄÄÇ      Å upload   Å  å
 *           å  Å install  Å        6å stats   å     2Ñ∑∑∑∑∑∑∑∑∑∑Ö  å
 *           å  Ñ∑∑∑∑∑∑∑∑∑∑Ö5        å list    å                    å
 *         local                     å report  å                 cloud
 *           å                       å check   å                    å
 *           å  É∑∑∑∑∑∑∑∑∑∑Ç4        å audit   å                    å
 *           å  Å withdraw Å         å fix     å     2É∑∑∑∑∑∑∑∑∑∑Ç  å
 *           å  Å clear    Å         ÑÄÄâÄâÄâÄÄÖ      Å restore  Å  å
 *           å  Å remove   áÄÄ<<<ÄÄÄÄÄÄÄÖ Å ÑÄÄÄÄ<<<ÄÄÜ download Å  å
 *           å  Å extract  Å              Å           Ñ∑∑∑∑∑∑∑∑∑∑Ö  å
 *           å  Ñ∑∑∑∑∑∑∑∑∑∑Ö              Å                         å
 *           Ñ∑∑∑∑                  6ÉÄÄÄÄàÄÄÄÄÇ                ∑∑∑∑Ö
 *                                   å gather  å
 *                                   å daemon  å
...
static const tOPTS   s_opts [MAX_OPTS] = {
   ...
   /*---(incomming 5)--------------------------*/
   { 'i' , "verify"   , "v—V", "verify local file for correctness"            ,   4, '-', YJOBS_LOCAL    },
   { 'i' , "local"    , "lÚL", "report on local file content"                 ,   5, '-', YJOBS_LOCAL    },
   { 'i' , "register" , "bÈB", "verify local file and register centrally"     ,   6, '-', YJOBS_LOCAL    },
   { 'i' , "update"   , "u˚U", "update central with single local file"        ,   7, '-', YJOBS_LOCAL    },
   { 'i' , "install"  , "iI", "verify local file, then update centrally"     ,   8, '-', YJOBS_LOCAL    },
   /*---(maintain 6)---------------------------*/
   { 'm' , "stats"    , "#∑∑", "information about database and environment"   ,  10, '-', YJOBS_CENTRAL  },
   { 'm' , "list"     , "=∑∑", "list of executable content"                   ,  11, '-', YJOBS_CENTRAL  },
   { 'm' , "report"   , "mÛM", "report on central database"                   ,  12, '-', YJOBS_CENTRAL  },
   { 'm' , "check"    , "c˝C", "check central file for correctness"           ,  13, '-', YJOBS_CENTRAL  },
   { 'm' , "audit"    , "aËA", "audit central environment and all its files"  ,  14, '-', YJOBS_CENTRAL  },
   { 'm' , "fix"      , "f¸F", "audit central environment and fix issues"     ,  15, '-', YJOBS_CENTRAL  },
   { 'm' , "only"     , "oˆO", "run on single central data file"              ,  16, 'y', YJOBS_CENTRAL  },
   /*---(epic)---------------------------------*/
   { 'e' , "backup"   , "kÒK", "backup the central system"                    ,  18, '-', YJOBS_CENTRAL  },
   { 'e' , "restore"  , "t˙T", "restore the central system from backup"       ,  19, '-', YJOBS_CENTRAL  },
   { 'e' , "purge"    , "wˇW", "wipe clean entire central system"             ,  20, '-', YJOBS_CENTRAL  },
   /*---(elsewhere)----------------------------*/
   { 'e' , "upload"   , "yÔY", "from elsewhere to central"                    ,  22, '-', YJOBS_CENTRAL  },
   { 'e' , "download" , "zÌZ", "from central to elsewhere"                    ,  23, '-', YJOBS_CENTRAL  },
   /*---(outgoing 4)---------------------------*/
   { 'o' , "withdraw" , "q˛Q", "unregister centrally"                         ,  25, '-', YJOBS_CENTRAL  },
   { 'o' , "clear"    , "xıX", "clear file from central location"             ,  26, '-', YJOBS_CENTRAL  },
   { 'o' , "remove"   , "r¯R", "unregister and clear from central location"   ,  27, '-', YJOBS_CENTRAL  },
   { 'o' , "extract"  , "eÏE", "extract a central file to local copy"         ,  28, '-', YJOBS_CENTRAL  },
   /*---(execution)----------------------------*/
   { 'g' , "gather"   , "gÍG", "execute system-wide data gather"              ,  30, '-', YJOBS_NEITHER  },
   { 'g' , "daemon"   , "dÎD", "execute specific file in daemon-mode"         ,  31, 'y', YJOBS_NEITHER  },
   { 'g' , "prickly"  , "p˜P", "execute specific file in prickly daemon-mode" ,  32, 'y', YJOBS_NEITHER  },
   { 'g' , "normal"   , "nÙN", "execute specific file in normal-mode"         ,  33, 'y', YJOBS_NEITHER  },
   { 'g' , "strict"   , "s˘S", "execute specific file in strict normal-mode"  ,  34, 'y', YJOBS_NEITHER  },
   { 'g' , "reload"   , "hÓH", "send signal to reload daemon"                 ,  35, '-', YJOBS_NEITHER  },
   /*---(unit testing)-------------------------*/
...
yjobs_args_init         (char *r_runas, char *r_mode, char *r_file)
{
   ...
   for (i = 0; i < MAX_OPTS; ++i) {
      ...
      DEBUG_YJOBS  yLOG_complex ("current"   , "%2d, %s, %s", i, s_opts [i].levels, s_opts [i].option);
      if (p [0] == '\0')  break;
      if (rc >= 0)  rc = yjobs_args__single (p, s_opts [i].cat, 0, s_opts [i].run, s_opts [i].where);
      DEBUG_YJOBS  yLOG_value   ("0"         , rc);
      if (rc >= 0)  rc = yjobs_args__single (p, s_opts [i].cat, 1, s_opts [i].run, s_opts [i].where);
      DEBUG_YJOBS  yLOG_value   ("1"         , rc);
      if (rc >= 0)  rc = yjobs_args__single (p, s_opts [i].cat, 2, s_opts [i].run, s_opts [i].where);
...
{ 
   ...
   if (r_file  != NULL)   ystrlcpy (r_file, myJOBS.m_file, LEN_PATH);
   /*---(defense)------------------------*/
   DEBUG_YJOBS   yLOG_complex ("runas"     , "%c  Â%sÊ", myJOBS.m_runas, g_valid);
   ...
   --rce;  if (strlen (a_arg) <= 4) {
      yURG_err ('F', "option Â%sÊ too short, must be > 4 chars", a_arg);
      DEBUG_YJOBS  yLOG_exitr   (__FUNCTION__, rc);
      return rce;
   }
   --rce;  if (strncmp (a_arg, "--", 2) != 0) {
      yURG_err ('F', "option Â%sÊ must start with the prefix Â--Ê", a_arg);
      DEBUG_YJOBS  yLOG_exitr   (__FUNCTION__, rc);
      return rce;
   }
   /*> --rce;  if (myJOBS.m_mode != '-') {                                                               <* 
    *>    yURG_err ('F', "run action already set (%c), can not update to Â%sÊ", myJOBS.m_mode, a_arg);   <* 
    *>    DEBUG_YJOBS  yLOG_exitr   (__FUNCTION__, rc);                                                  <* 
    *>    return rce;                                                                                    <* 
    *> }                                                                                                 <*/
   /*---(complete)-----------------------*/
...
yjobs_args__find        (char *a_arg, char *n, char *r_runas, char *r_noise)
{
   ...
   --rce;  if (c == '?') {
      yURG_msg ('-', "action Â%sÊ not found in database, maybe program specific", a_arg);
      DEBUG_YJOBS  yLOG_exit    (__FUNCTION__);
      return c;
   }
   /*---(handle not-allowed)-------------*/
   DEBUG_YJOBS  yLOG_char    ("c"         , c);
   --rce;  if (c == '∑') {
      yURG_err ('F', "action Â%sÊ, (%c) not allowed", a_arg, a_arg [2]);
      DEBUG_YJOBS  yLOG_exitr   (__FUNCTION__, rce);
      return '∑';
   }
   /*---(testing)------------------------*/
...
yJOBS_argument          (int *b_pos, cchar *a_arg, cchar *a_next, char *r_runas, char *r_mode, char *r_file)
{
   ...
   --rce;  if (f == '∑') {
      yURG_err ('F', "action Â%sÊ not allowed for runas (%c)", a_arg, myJOBS.m_runas);
      DEBUG_YJOBS  yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   ...
   /*---(handle file flag)---------------*/
   --rce;  if (f != 'F') {
      yURG_err ('F', "action Â%sÊ not configured correctly", a_arg);
   ...
   --rce;  if (a_next == NULL) {
      ...
      yURG_err ('F', "action Â%sÊ requires a file name as an argument", a_arg);
      yjobs_args__clearmode (r_runas, r_mode, r_file);
      DEBUG_YJOBS  yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
...
yjobs_args__unit        (char *a_question)
{
   ...
   if      (strcmp (a_question, "args"          )  == 0) {
      sprintf (s, "%2dÂ%.35sÊ", strlen (g_silent) , g_silent);
      sprintf (t, "%2dÂ%.30sÊ", strlen (g_confirm), g_confirm);
      sprintf (u, "%2dÂ%.30sÊ", strlen (g_verbose), g_verbose);
